...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func spawnSpaceJunk(){
        ...
        var model = test1_1()
        var randomSjY = CGFloat(Float.random(in: 36 ..< Float(self.size.height - 36)))
        do{
            try randomSjY = CGFloat(model.prediction(input: test1_1Input(verticalAcceleration: (self.zEff * 30000) / Double(self.player.physicsBody!.mass), verticalSpeed: Double((self.player.physicsBody?.velocity.dy)!), verticalPosition: Double(self.player.position.y))).verticalFuturePosition)
        } catch {
            print("error")
        }
    ...
    func runTimer() {
        var timer = Timer.scheduledTimer(withTimeInterval: 0.1, repeats: true) { _ in
            JsonManager.shared.save(verticalPosition: Double(self.player.position.y), verticalSpeed: Double((self.player.physicsBody?.velocity.dy)!), verticalAcceleration: (self.zEff * 30000) / Double(self.player.physicsBody!.mass))
        }
    }
}
